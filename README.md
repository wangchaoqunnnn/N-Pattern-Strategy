...
- **多源冗余 + 自动切换**（地址与顺序全部在 `server/providers.json`，改配置即可换源）：

  | 用途 | 源顺序（前一个不可用自动切下一个） |
  |---|---|
  | 实时行情 | 新浪 hq → 腾讯 qt → 腾讯备用域名 → 东方财富 ulist |
  | 日K | 腾讯 proxy → 腾讯 web → 东方财富 kline → 新浪 money |
  | 股票列表 | 新浪沪深京A股 → 东方财富 clist（B股：东财 → 代码段探测） |

  切换策略：记录每个源的成功/失败，**最近可用源优先**（避免每轮都先撞已失效的主源），**10 分钟后自动回探主源**，主源恢复即自动切回；`/api/diag` 与 `tools/providers_check.py` 可查看各源连通性与当前优先源。
- **日K**：不复权，与实盘价格一致；覆盖沪深、指数、**北交所**与 **B股**。
- **股票列表**：新浪沪深京A股列表为主源（沪主板/深主板/科创板/创业板/北交所）；**B股(沪B/深B)** 由东方财富列表补充，东方财富不可达时自动改用 **B股代码段探测**（新浪行情枚举 900901-900999 / 200002-200999，实测纳入 91 只）；主源被限流时保留本地缓存继续运行。
