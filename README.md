...
- **行情（实时）**：新浪批量行情为主源；若云服务器IP被新浪返回 403/超时，**自动切换腾讯行情补齐**（沪深300/北交所/B股通用），保证盘中"今日K线"合成与买卖执行不断档。
- **日K**：腾讯 proxy 主源（不复权，与实盘价格一致，沪深/指数/**北交所**通用）→ 腾讯 web 备源 → 新浪 money 回退源；指数：沪深300/上证指数。
